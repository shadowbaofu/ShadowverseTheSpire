...
public class Alberta extends CustomCard {
    ...
    public void use(AbstractPlayer abstractPlayer, AbstractMonster m) {
        addToBot(new SFXAction("Alberta"));
        addToBot(new DamageAction(m, new DamageInfo(abstractPlayer, this.damage, this.damageTypeForTurn), AbstractGameAction.AttackEffect.SLASH_DIAGONAL));
        addToBot(new ApplyPowerAction(abstractPlayer, abstractPlayer, new AlbertaPower(abstractPlayer, this.upgraded)));
    }
