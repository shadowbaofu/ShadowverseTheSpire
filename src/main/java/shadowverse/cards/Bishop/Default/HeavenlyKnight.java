...
public class HeavenlyKnight extends CustomCard {
    ...
    public void use(AbstractPlayer abstractPlayer, AbstractMonster m) {
        addToBot(new SFXAction("HeavenlyKnight"));
        addToBot(new GainBlockAction(abstractPlayer, this.block));
        if (m != null)
            addToBot(new VFXAction(new VerticalImpactEffect(m.hb.cX + m.hb.width / 4.0F, m.hb.cY - m.hb.height / 4.0F)));
        addToBot(new DamageAction(m, new DamageInfo(abstractPlayer, this.block, this.damageTypeForTurn), AbstractGameAction.AttackEffect.BLUNT_HEAVY));
    }
