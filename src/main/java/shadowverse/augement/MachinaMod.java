...
public class MachinaMod extends AbstractAugment {

    //The ID should be prefixed with your modID.
    public static final String ID = "shadowverse:MachinaMod";
    ...
    public void onInitialApplication(AbstractCard card) {
        if (!card.hasTag(AbstractShadowversePlayer.Enums.MACHINE)) {
            card.tags.add(AbstractShadowversePlayer.Enums.MACHINE);
        }
    }

    public float modifyBaseDamage(float damage, DamageInfo.DamageType type, AbstractCard card, AbstractMonster target) {
        return card.baseDamage > 1 ? damage * 0.75F : damage;
    }

    public float modifyBaseBlock(float block, AbstractCard card) {
        return card.baseBlock > 1 ? block * 0.75F : block;
    }

    @Override
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        addToBot(new MakeTempCardInHandAction(new ProductMachine(), 1));
    }
